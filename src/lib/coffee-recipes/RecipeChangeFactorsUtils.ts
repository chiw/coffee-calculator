import type { CoffeeParametersConfig, RecipeChangeFactors, RecipeChangeStatus, StepControls } from "./CoffeeRecipeTypes"

export const compareRecipeChangeFactors = (recipeChangeFactors: RecipeChangeFactors, defaultRecipeChangeFactors: RecipeChangeFactors): RecipeChangeStatus => {
    if(recipeChangeFactors === undefined && defaultRecipeChangeFactors === undefined) return <RecipeChangeStatus> {
...
const stepsDurationInSecondsHasChanged = (stepsDurationInSeconds: number[], defaultStepsDurationInSeconds: number[]): boolean => {
    ...
    if(stepsDurationInSeconds === undefined || defaultStepsDurationInSeconds === undefined) return true;
