...
export const createRecipeMetaInfo = (
	brandName: string,
	dripperName: string,
	recipeName: string,
	recipeId: string,
	...
	metaInfoArr.push(<MetaInfo>{
		name: MetaInfoKey.path,
		value: brandName + '/' + dripperName + '/' + recipeName
	});
...
export const metaInfoIsMatched = (
	metaInfos: MetaInfos,
	...
	if (metaInfos && inSearchStr) {
		const match =
			metaInfos.values
				...
				.filter((metaInfo) =>
					exactWording ? metaInfo.value == inSearchStr : metaInfo.value.includes(inSearchStr)
				).length > 0;
...
const filterMetaInfosBySearchParam = (
...
export const filterMetaInfosBySearchParams = (
	inMetaInfoArr: MetaInfos[],
	searchParams: MetaInfosSearchParam[]
): MetaInfos[] => {
