...
export const createStepControls = (
...
const getSelectableFlags = (
	stepAdjustments: StepAdjustmentsConfig
): StepAdjustmentSelectableFlagConfig[] => {
	return [
		<StepAdjustmentSelectableFlagConfig>{
			stepAdjustmentName: StepAdjustment.TWO_STEPS_RATIOS,
			isSelectable: stepAdjustments.twoStepsRatios?.canEdit
		},
		<StepAdjustmentSelectableFlagConfig>{
			stepAdjustmentName: StepAdjustment.POUR_DIVISIONS,
			isSelectable: stepAdjustments.pourDivisions?.canEdit
		}
	];
};

export const stepOptionsAreSelectable = (
...
export const createStepsFromStepAdjustments = (
	stepsAdjustments: StepAdjustmentsConfig,
	stepAdjustmentSelectedOptions: StepAdjustmentSelectedOptionConfig[]
) => {
	...
	stepsAdjustments.order.forEach((adjustment) => {
		...
		if (adjustment === StepAdjustment.TWO_STEPS_RATIOS) {
			steps = steps.concat(
				createStepsFromTwoStepsRatios(
					stepsAdjustments.twoStepsRatios,
					options ? options[0].option : null
				)
			);
		}
		...
		if (adjustment === StepAdjustment.POUR_DIVISIONS) {
			steps = steps.concat(
				createStepsFromPourDivisions(
					stepsAdjustments.pourDivisions,
					options ? options[0].option : null
				)
			);
		}
...
const createStepsFromTwoStepsRatios = (
...
const createStepsFromPourDivisions = (
	pourDivisionsConfig: PourDivisionsConfig,
	selectedOption?: string
): StepConfig[] => {
...
export const recreateStepsWithStepControl = (
	...
	stepAdjustments.order.forEach((adjustmentName) => {
		...
		if (adjustmentName === StepAdjustment.TWO_STEPS_RATIOS) {
			steps = steps.concat(
				createStepsFromTwoStepsRatios(stepAdjustments.twoStepsRatios, selectedOption?.option)
			);
		}

		if (adjustmentName === StepAdjustment.POUR_DIVISIONS) {
			steps = steps.concat(
				createStepsFromPourDivisions(stepAdjustments.pourDivisions, selectedOption?.option)
			);
		}
	});
