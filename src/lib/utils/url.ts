...
export const dirtyFixFullUrlPath = (path: string): string => {
    ...
    if(path.startsWith(githubPageRepoName + githubPageRepoName)) {
        console.log('dirtyFixFullUrlPath [' + path + '] contains [' + githubPageRepoName + githubPageRepoName +'] remove the first occurance');
        result = path.replace(githubPageRepoName + githubPageRepoName, githubPageRepoName);
    }
