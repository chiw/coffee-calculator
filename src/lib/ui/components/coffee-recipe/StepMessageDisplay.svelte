...
<div class="grow ml-2">
	{#if step.stage}
		{#if isPauseStage(step.stage)}
			{#if step.swirl}
				<span class="text-sm"
					>{@html pourOverStageMessageKey(step.stage)() +
						' ' +
						(step.swirl ? '<b>(' + swirlMessageKey() + ')</b>' : '')}</span
				>
			{:else if step.stir}
				<span class="text-sm"
					>{@html pourOverStageMessageKey(step.stage)() +
						' ' +
						(step.stir ? '<b>(' + stirMessageKey() + ')</b>' : '')}</span
				>
			{:else}
	...
	{#if step.pouringTechnique}
		<span class="text-sm"
			>{@html ' (' + pouringTechniqueMessageKey(step.pouringTechnique)() + ')'}</span
		>
	{/if}
	<!-- <span>{@html "<br/>"}</span> -->
	...
	{#if step.showWaterTemperature}
		<span class="ml-0 text-sm"
			>{@html waterTemperatureMessageKey(step.stepWaterInfo.waterTemperature)}</span
		>
	{/if}
