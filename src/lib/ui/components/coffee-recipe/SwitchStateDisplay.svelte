<script lang="ts">
    ...
    let { switchState, highlightState = false, isImmersionDripperRecipe = false, showTimeframeEndTime = true, durationInSeconds }: SwitchStateDisplayProps = $props();
</script>
...
<div>
    {#if SwitchState.OPEN === switchState && isImmersionDripperRecipe}
        ...
        <div class=" w-12 font-bold text-xs bg-white text-black text-center">{switchStateMessage(switchState)}</div>
        {#if !showTimeframeEndTime}
        <div class=" w-12 font-bold text-xs bg-white text-black text-center border-b-1 border-black">{durationInSeconds}s</div>
        {/if}
    {:else if SwitchState.CLOSED === switchState && isImmersionDripperRecipe}
        ...
        <div class=" w-12 font-bold text-xs bg-black text-white text-center">{switchStateMessage(switchState)}</div>
        {#if !showTimeframeEndTime}
        <div class=" w-12 font-bold text-xs bg-black text-white text-center border-b-1 border-black">{durationInSeconds}s</div>
        {/if}
    {:else}
