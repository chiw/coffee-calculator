...
{#each stepAdjustmentAvailableOptions.options as availableOption, i }
    ...
    <div>
        ...
        <button class="flex cursor-pointer items-center justify-center rounded
            {availableOption === stepAdjustmentSelectedOption.option ? 'border border-black bg-black px-2 py-1 text-white' : 'border border-gray-300 bg-white px-2 py-1 text-gray-900' }
            hover:border-gray-900" 
            onclick={() => handleOnClick(availableOption)}
            >
            <span class="text-xs font-bold">{getStepAdjustmentOptionMessageKey(stepAdjustmentAvailableOptions.stepAdjustmentName, availableOption)}</span>
        </button>
    </div>
{/each}
</fieldset>
