<script lang="ts">
    ...
    function resetTimer() {
        ...
        stopwatch.reset = timerInSeconds;
    }
...
<div class="flex flex-col mt-2 mb-1">
    {#if steps}
        ...
        {:else}
            <div>
                <button class="w-24 bg-black hover:bg-slate-600 text-white rounded font-bold" onclick={startTimer}>{m.label_start_recipe()}</button>
            </div>
        {/if}
